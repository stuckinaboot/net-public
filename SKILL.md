...
`--encode-only` works with all netp write commands: `storage upload`, `token deploy`, `upvote token`, `upvote user`, `bazaar buy-listing`, `bazaar submit-listing`, `bazaar submit-offer`, `bazaar accept-offer`, `bazaar buy-erc20-listing`, `bazaar submit-erc20-listing`, `bazaar submit-erc20-offer`, `bazaar accept-erc20-offer`.
...
```
Submit each approval first, then the fulfillment (include `value` — it's the listing price in wei for NFT/ERC-20 listing buys; zero for ERC-20 offer accepts since payment is in WETH).
...
| **Bazaar (ERC-20)** | ERC-20 listings/offers are supported on Base (8453) and HyperEVM (999). ERC-20 commands use `--token-address` + `--token-amount` (raw bigint units). Listings pay in ETH; offers pay in WETH (offerer pre-approves WETH). |
...
- "Accept an ERC-20 offer (sell tokens for WETH)" → `netp bazaar accept-erc20-offer --order-hash 0x... --token-address 0x... --seller 0xMyAddr --chain-id 8453 --encode-only`
